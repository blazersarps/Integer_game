...
import { level_two_questions } from "../questions";
...
const questions = level_two_questions;
var score = 0;
var tries = 1;
var question = 1;
var questionsRecord = [];
...
export default class LevelTwoScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("question2", ["assets/audio/question2.mp3"]);
  ...
  create() {
    ...
    var question2 = this.sound.add("question2");
    ...
    this.nextButton.on("pointerup", () => {
      this.scene.start("second-video-scene", {
        playerName: this.playerName,
        characterColor: this.characterColor,
        previousScore: this.previousScore + score,
      });
      score = 0;
      tries = 1;
    });
    ...
    const addQuestions = () => {
      questions.forEach((item) => {
        ...
        var qtext = this.add.text(
          100,
          item.tall - 80,
          "Identify the numbers which are not integers in the following numbers",
          {
        ...
        answerOne2.setInteractive();
        answerTwo2.setInteractive();
        answerThree2.setInteractive();
        answerFour2.setInteractive();
        answerFive2.setInteractive();

        //Mouse onHover/PointerOver actions for possible Answers
        answerOne2.on("pointerover", () => {
          answerOne2.setScale(0.9, 0.9);
          question2.play();
        });
        answerTwo2.on("pointerover", () => {
          answerTwo2.setScale(0.9, 0.9);
          question2.play();
        });
        answerThree2.on("pointerover", () => {
          answerThree2.setScale(0.9, 0.9);
          question2.play();
        });
        answerFour2.on("pointerover", () => {
          answerFour2.setScale(0.9, 0.9);
          question2.play();
        });
        answerFive2.on("pointerover", () => {
          answerFive2.setScale(0.9, 0.9);
          question2.play();
        });

        //Mouse onPointerOut actions for possible Answers
        answerOne2.on("pointerout", () => {
          answerOne2.setScale(0.7, 0.7);
        });
        answerTwo2.on("pointerout", () => {
          answerTwo2.setScale(0.7, 0.7);
        });
        answerThree2.on("pointerout", () => {
          answerThree2.setScale(0.7, 0.7);
        });
        answerFour2.on("pointerout", () => {
          answerFour2.setScale(0.7, 0.7);
        });
        answerFive2.on("pointerout", () => {
          answerFive2.setScale(0.7, 0.7);
        });
        ...
        answerOne2.on("pointerup", () => {
          const heightToUse = item.tall + 500;
          const hasAnswer = istheAnswer(answerOne2.text);
            ...
            if (tries === 3) {
              personData.gameOvers = personData?.gameOvers
                ? [
                    ...
                    { no: question, tries, time: new Date(), Level: 2 },
                  ]
                : [{ no: question, tries, time: new Date(), Level: 2 }];
              localforage
              ...
              answerOne2.setVisible(false);
              answerTwo2.setVisible(false);
              answerThree2.setVisible(false);
              answerFour2.setVisible(false);
              answerFive2.setVisible(false);
              ...
              Swal.fire({
                position: "top",
                text: "Decimals and Fractions are not Integers",
                width: 300,
              });
            }
          }
        });
        answerTwo2.on("pointerup", () => {
          const heightToUse = item.tall + 500;
          const hasAnswer = istheAnswer(answerTwo2.text);
          const point = getPoint(tries);
          if (hasAnswer) {
            this.tweens.add({
              targets: player,
        ...
        answerThree2.on("pointerup", () => {
          const heightToUse = item.tall + 500;
          const hasAnswer = istheAnswer(answerThree2.text);
          const point = getPoint(tries);
          if (hasAnswer) {
            this.tweens.add({
              targets: player,
        ...
        answerFour2.on("pointerup", () => {
          const heightToUse = item.tall + 500;
          const hasAnswer = istheAnswer(answerFour2.text);
          const point = getPoint(tries);
          if (hasAnswer) {
            this.tweens.add({
              targets: player,
        ...
        answerFive2.on("pointerup", () => {
          const heightToUse = item.tall + 500;
          const hasAnswer = istheAnswer(answerFive2.text);
          const point = getPoint(tries);
          if (hasAnswer) {
            this.tweens.add({
              targets: player,
    ...
    this.physics.add.overlap(player, platformEnd, () => {
      if (score >= 600) {
        ...
        personData.levelTwoData = {
          score,
          totalTimeSpent,
          questionsRecord,
        };
        personData.levelTwoComplete = true;
        localforage
          .setItem(this.playerName, personData)
          .then(function () {
