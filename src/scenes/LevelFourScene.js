...
import { level_four_questions } from "../questions";
const questions = level_four_questions;
var score = 0;
var tries = 1;
var question = 1;
var questionsRecord = [];
...
export default class LevelFourScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("question4", ["assets/audio/question4.mp3"]);
  ...
  create() {
    ...
    var question4 = this.sound.add("question4");
    ...
    this.trophy = this.add.image(250, 23800, "trophy").setVisible(false);
    this.badge = this.add
      .image(900, 23600, "badge4")
      .setScale(0.5)
      .setVisible(false);
            ...
            if (tries === 3) {
              personData.gameOvers = personData?.gameOvers
                ? [
                    ...
                    { no: question, tries, time: new Date(), Level: 4 },
                  ]
                : [{ no: question, tries, time: new Date(), Level: 4 }];
              localforage
              ...
              Swal.fire({
                position: "top",
                text: "The numbers to the left side of the number line gets smaller and smaller",
                width: 300,
              });
            }
          }
    ...
    this.physics.add.overlap(player, platformEnd, () => {
      if (score >= 600) {
        ...
        if (this.previousScore + score === 4000) {
          this.trophy.setVisible(true);
        }
        player.anims.play("win");
        personData.levelFourData = {
          score,
          totalTimeSpent,
          questionsRecord,
        };
        personData.levelFourComplete = true;
        localforage
          .setItem(this.playerName, personData)
          .then(function () {
